...
import { Flex, Center, Text, Switch } from "@chakra-ui/react";
...
export async function getServerSideProps({ params }) {
  ...
  const [res1, res2, res3, res4, res5, res6] = await Promise.all([
    ...
    fetch(
      `https://financialmodelingprep.com/api/v3/key-metrics/${symbol}?limit=10&apikey=${apikey}`
    ).then((response) => response.json()),
    fetch(
      `https://financialmodelingprep.com/api/v3/historical-price-full/${symbol}?from=${fiveYearsAgoForApi}&to=${yesterdayForApi}&apikey=${apikey}`
    ).then((response) => response.json()),
    fetch(
      `https://financialmodelingprep.com/api/v3/quote/${symbol}?apikey=${apikey}`
    ).then((response) => response.json()),
    fetch(
      `https://financialmodelingprep.com/api/v3/income-statement/${symbol}?period=quarter&limit=10&apikey=${apikey}`
    ).then((response) => response.json()),
    fetch(
      `https://financialmodelingprep.com/api/v3/key-metrics/${symbol}?period=quarter&limit=10&apikey=${apikey}`
    ).then((response) => response.json()),
  ]);

  const plData = res1.length
    ? res1.map((each) => ({
        ...
        operatingExpense:
          each.revenue - each.costOfRevenue - each.operatingIncome,
        operatingExpenseR:
          ((each.revenue - each.costOfRevenue - each.operatingIncome) * 100) /
          each.revenue,
        nonOperatingExpense: each.operatingIncome - each.netIncome,
        nonOperatingExpenseR:
          ((each.operatingIncome - each.netIncome) * 100) / each.revenue,
        grossProfit: each.revenue - each.costOfRevenue,
        grossProfitR:
          ((each.revenue - each.costOfRevenue) * 100) / each.revenue,
        ...
        sellingAndMarketingExpensesR:
          (each.sellingAndMarketingExpenses * 100) / each.operatingExpenses,
        rd: each.researchAndDevelopmentExpenses,
        rdR:
          (each.researchAndDevelopmentExpenses * 100) / each.operatingExpenses,
        sga: each.operatingExpenses - each.researchAndDevelopmentExpenses,
        sgaR:
          ((each.operatingExpenses - each.researchAndDevelopmentExpenses) *
            100) /
        ...
        ebitdaR:
          ((each.netIncome +
            each.incomeTaxExpense +
            each.interestExpense +
            each.depreciationAndAmortization) *
            100) /
          each.revenue,
  ...
  const keyMetrics = res2.length
    ? res2.map((keyMetric) => ({
        date: keyMetric.date.split("-"),
        operatingCashFlowPerShare:
          Math.round(keyMetric.operatingCashFlowPerShare * 100) / 100,
        freeCashFlowPerShare:
          Math.round(keyMetric.freeCashFlowPerShare * 100) / 100,
      }))
    : [];

  const historicalPrice = res3.symbol
    ? res3.historical.map((dailyPrice) => ({
...
export default function IncomeStatement({
  ...
  if (plDataQ == null || keyMetrics == null) {
    return (
      <Layout>
        ...
        <Flex direction="column" mx="2%" my="8%">
          <Text fontWeight="bold" fontSize="calc(6px + 4vmin)">
            🙇‍ Sorry, No Data...
          </Text>
          <Text fontSize="calc(6px + 2vmin)" m="1%">
            ・ 入力したティッカーに間違いは無いですか？(ex. ”APPL")
          </Text>
          <Text fontSize="calc(6px + 2vmin)" m="1%">
            ・ 個別銘柄以外のETF等は現状対応していません。
          </Text>
          <Text fontSize="calc(6px + 2vmin)" m="1%">
            ・
            上場直後で決算データが揃っていない銘柄は表示されない場合があります。
          </Text>
        </Flex>
      </Layout>
    );
    ...
    const profitData =
      plData.length &&
      plDataQ.length &&
      keyMetrics.length == plData.length &&
      isAnnual === true
        ? plData
            .map((each, index) => {
              return {
                ...
                operatingIncome:
                  Math.round((each.operatingIncome / 1000000) * 10) / 10,
                ...
                nonOperatingExpenseR:
                  Math.round(each.nonOperatingExpenseR * 10) / 10,
                ...
                weightedAverageShsOutDil:
                  each.weightedAverageShsOutDil / 1000000,
                eps: Math.round(each.eps * 100) / 100,
                revenuePerShare: Math.round(each.revenuePerShare * 100) / 100,
                operatingCashFlowPerShare:
                  Math.round(
                    keyMetrics[index].operatingCashFlowPerShare * 100
                  ) / 100,
                freeCashFlowPerShare:
                  Math.round(keyMetrics[index].freeCashFlowPerShare * 100) /
                  100,
              };
            })
            .reverse()
            ...
            .map((each, index) => {
              if (keyMetricsQ.length == plDataQ.length) {
                return {
                  ...
                  revenue: Math.round((each.revenue / 1000000) * 10) / 10,
                  grossProfit:
                    Math.round((each.grossProfit / 1000000) * 10) / 10,
                  grossProfitR: Math.round(each.grossProfitR * 10) / 10,
                  operatingIncome:
                    Math.round((each.operatingIncome / 1000000) * 10) / 10,
                  operatingIncomeR: Math.round(each.operatingIncomeR * 10) / 10,
                  costOfRevenue:
                    Math.round((each.costOfRevenue / 1000000) * 10) / 10,
                  costOfRevenueR: Math.round(each.costOfRevenueR * 10) / 10,
                  operatingExpense: each.operatingExpense / 1000000,
                  operatingExpenseR:
                    Math.round(each.operatingExpenseR * 10) / 10,
                  nonOperatingExpense: each.nonOperatingExpense / 1000000,
                  nonOperatingExpenseR:
                    Math.round(each.nonOperatingExpenseR * 10) / 10,
                  netIncome: Math.round((each.netIncome / 1000000) * 10) / 10,
                  ...
                  operatingCashFlowPerShare:
                    Math.round(
                      keyMetricsQ[index].operatingCashFlowPerShare * 100
                    ) / 100,
                  freeCashFlowPerShare:
                    Math.round(keyMetricsQ[index].freeCashFlowPerShare * 100) /
                    100,
                };
              } else {
                return {
    ...
    const profitStructureKeyword = isPercent === true ? "profitR" : "profit";
      ...
      ? {
          ...
          data: [
            ...
            },
            {
              title: "Gross Profit",
              value:
                profitData[profitData.length - 1].grossProfit.toLocaleString(),
              unit_forth: "$",
              unit_back: "",
            },
            {
            ...
            },
            {
              title: "Net Income",
              value:
                profitData[profitData.length - 1].netIncome.toLocaleString(),
              unit_forth: "$",
              unit_back: "",
            },
          ],
    ...
    const operatingCashFlowPerShare_highlight = profitData[
      profitData.length - 1
    ].operatingCashFlowPerShare
      ? profitData[
          profitData.length - 1
        ].operatingCashFlowPerShare.toLocaleString()
      : "";

    const freeCashFlowPerShare_highlight = profitData[profitData.length - 1]
      .freeCashFlowPerShare
      ? profitData[profitData.length - 1].freeCashFlowPerShare.toLocaleString()
      : "";

    const highlightData_pershare = profitData[profitData.length - 1]
      .revenuePerShare
      ? {
          symbol: value,
          period: profitData[profitData.length - 1].date,
          data: [
            ...
            },
            {
              ...
              value:
                profitData[
                  profitData.length - 1
                ].revenuePerShare.toLocaleString(),
              unit_forth: "$",
              unit_back: "",
            },
            {
    ...
    return (
      <Layout>
        ...
        <Flex direction="column" my="3%">
              ...
              <Flex w="85%" wrap="wrap">
                {secData.map((e) => {
                  return (
                    ...
                    >
                      <a href={e.annualReport} target="_blank">
                        {e.data}
                      </a>
                    </Flex>
                  );
              ...
              <Flex w="85%" wrap="wrap">
                {secDataQ.map((e) => {
                  return (
                    ...
                    >
                      <a href={e.quarterlyReport} target="_blank">
                        {e.data}
                      </a>
                    </Flex>
                  );
          ...
          {profitData.length < 3 ? (
            <Text m="2%" color="#fb8fb9" fontSize="sm">
              ⚠️Sorry, the number of data may not be plenty enough to research⚠️
            </Text>
          ) : null}
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Profit Structure</Text>
                  </Center>
                  <AreaRechart
                    data={profitData}
                    keyword={profitStructureKeyword}
                    color={chartColor.rev}
                  />
                  <Text
              ...
              >
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Revenue</Text>
                  </Center>
                  <BarRechart
                    data={profitData}
                    title={["revenue"]}
                    color={[chartColor.rev[0]]}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Gross Profit</Text>
                  </Center>
                  {isPercent === true ? (
                    <LineRechart
                      data={profitData}
                      title={["grossProfitR"]}
                      color={[chartColor.rev[1]]}
                    />
                  ) : (
                    <BarRechart
                      data={profitData}
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Operating Income</Text>
                  </Center>
                  {isPercent === true ? (
                    <LineRechart
                      data={profitData}
                      title={["operatingIncomeR"]}
                      color={[chartColor.rev[2]]}
                    />
                  ) : (
                    <BarRechart
                      data={profitData}
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Net Income</Text>
                  </Center>
                  {isPercent === true ? (
                    <LineRechart
                      data={profitData}
                      title={["netIncomeR"]}
                      color={[chartColor.rev[3]]}
                    />
                  ) : (
                    <BarRechart
                      data={profitData}
            ...
            >
              ...
              >
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">
                      ${value}&nbsp;From Net Income to EBITDA
                    </Text>
                  </Center>
                  <AreaRechart
                    data={profitData}
                    keyword="ebitda"
                    color={chartColor.ebitda}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
              ...
              >
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;EBITDA Margin</Text>
                  </Center>
                  <LineRechart
                    data={profitData}
                    title={["ebitdaR"]}
                    color={[chartColor.ebitda[0]]}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Interest Expense</Text>
                  </Center>
                  <BarRechart
                    data={profitData}
                    title={["interestExpense"]}
                    color={[chartColor.ebitda[1]]}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;EPS</Text>
                  </Center>
                  <BarRechart
                    data={profitData}
                    title={["eps"]}
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">${value}&nbsp;Revenue Per Share</Text>
                  </Center>
                  <BarRechart
                    data={profitData}
                    title={["revenuePerShare"]}
                    color={[chartColor.perShare[1]]}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
                ...
                >
                  <Center pt="2%">
                    <Text fontSize="12px">
                      ${value}&nbsp;Operating Cash Flow Per Share
                    </Text>
                  </Center>
                  <BarRechart
                    data={profitData}
                    title={["operatingCashFlowPerShare"]}
                    color={[chartColor.perShare[2]]}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
                ...
                >
                  ...
                  <BarRechart
                    data={profitData}
                    title={["freeCashFlowPerShare"]}
                    color={[chartColor.perShare[3]]}
                  />
                  <Text
                    fontSize="calc(2px + 1vmin)"
                    align="right"
